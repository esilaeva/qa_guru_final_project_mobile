...
помощи бота. Локальный запуск ощуцествляется в эмуляторе `Android Studio` при помощи `Appium Server GUI`
и `Appium Inspector`. Удаленный запуск на реальных мобильных усройствах осуществляется в `Browserstack`. Так же
реализована интеграция с `Allure TestOps` и `Jira`.
